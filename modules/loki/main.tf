...
resource "helm_release" "loki_stack" {
  ...
  values = [
    templatefile("${path.root}/helm-templates/loki/values.yaml", local.values_loki)
  ]

  depends_on = [time_sleep.wait_for_cleanup]
}

resource "kubectl_manifest" "loki_role" {
  validate_schema = false
  yaml_body       = templatefile("${path.root}/helm-templates/loki/role.yaml", local.values_loki)
}

resource "kubectl_manifest" "loki_rolebinding" {
  validate_schema = false
  yaml_body       = templatefile("${path.root}/helm-templates/loki/rolebinding.yaml", local.values_loki)
}
